...
public class AudioManager : MonoBehaviour 
{
	...
	void PlaySfx(string pSoundID, float pDelay = 0)
	{
    ...
    /// <returns></returns>
	AudioSource PlaySfxLooped(string pSoundID, float pDelay = 0)
	{
		if(mAudioDataDic.ContainsKey(pSoundID))
		{
			var aAudioData = mAudioData[mAudioDataDic[pSoundID]];
	...
	void Fade(string pSoundID, float pFadeTimer, bool pFadeOut, float pDelay = 0)
	{
		if(!mAudioSrcPlaying.ContainsKey(pSoundID))
		{
			return;
	...
	void SetMixerGroupVolume(string pMixerGroupName, float pVolume)
	{
	...
	bool IsPlaying(string pSoundID)
	{
        if (!mAudioSrcPlaying.ContainsKey(pSoundID))
        {
            return false;
    ...
    /// </summary>
    /// <param name="pSoundID">name of sound</param>
    /// <param name="pDelay">delay in starting</param>
	static public void PlaySFX(string pSoundID, float pDelay = 0)
	{
		mInstance.PlaySfx(pSoundID, pDelay);
	}


    /// <summary>
    /// Function to play sfx looped
    /// </summary>
    /// <param name="pSoundID">name of sound</param>
    /// <param name="pDelay">delay in sound</param>
    /// <returns></returns>
	static public AudioSource PlaySFXLooped(string pSoundID, float pDelay = 0)
	{
		return mInstance.PlaySfxLooped(pSoundID, pDelay);
	}

    /// <summary>
    /// Plays music
    /// </summary>
    /// <param name="pSoundID">music name</param>
    /// <param name="pLoop">if true then loops</param>
    /// <param name="pDelay">delay in starting</param>
    /// <returns></returns>
    ...
    /// <summary>
    /// Fades the given sound id
    /// </summary>
    /// <param name="pSoundID">sound name</param>
    /// <param name="pFadeTimer">time to fade</param>
    /// <param name="pFadeOut">if true then fades out</param>
    /// <param name="pDelay">delay in fading</param>
	static public void FadeSound(string pSoundID, float pFadeTimer, bool pFadeOut, float pDelay = 0)
	{
        if (mInstance != null)
        {
            mInstance.Fade(pSoundID, pFadeTimer, pFadeOut, pDelay);
        }
    ...
    /// </summary>
    /// <param name="pMixerGroupName">group name</param>
    /// <param name="pVolume">volume</param>
	static public void SetMixerGrpVolume(string pMixerGroupName, float pVolume)
	{
        if (mInstance != null)
        {
            mInstance.SetMixerGroupVolume(pMixerGroupName, pVolume);
        }
	}
    ...
    /// </summary>
    /// <param name="pSoundID">sound name</param>
    /// <returns>true if sound playing</returns>
	static public bool IsSoundPlaying(string pSoundID)
	{
        if (mInstance != null)
        {
            return mInstance.IsPlaying(pSoundID);
        }
		return false;
	}
    ...
    /// </summary>
    /// <param name="pSoundID">sound name</param>
	static public void StopSound(string pSoundID)
	{
        if (mInstance != null)
        {
            mInstance.StopSfx(pSoundID);
        }
	}
}
